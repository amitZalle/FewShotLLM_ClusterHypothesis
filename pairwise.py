...
###### CHANGED - added mission param and different prompt for clustering mission
class PairwiseLlmRanker(LlmRanker):
    def __init__(self, model_name_or_path,
        ...
        if self.mission == 'retrieval':
            self.prompt = """Given the following query: "{query}", 
                            choose which passage is **more relevant** to the query. 
                            
                            Passage A: "{doc1}"
                            Passage B: "{doc2}"
                            
                            You MUST choose exactly ONE: "Passage A" or "Passage B".  
                            
                            IMPORTANT:  
                            - Choose randomly if both passages are equally relevant.  
                            - Respond ONLY with "Passage A" or "Passage B".  
                            - No colons, no punctuation—EXACT match required.
                            """

        elif self.mission == 'clustering':
            self.prompt = """Given the following reference passage: "{query}", 
                            choose which passage is **more similar** to the reference passage. 
            ...
            raise ValueError(f"No such mission {self.method}")
    ...
    # CHANGED - changed so func returns original doc-text instead of None
    def rerank(self, query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        ...
        for i, doc in enumerate(ranking[:self.k]):
            ...
            results.append(SearchResult(docid=doc.docid, score=-rank, text=id_to_text[doc.docid]))
            rank += 1
