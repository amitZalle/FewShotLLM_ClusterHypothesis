...
### CHANGED - added new mission param and different prompts for clustering missions.
...
def create_permutation_instruction_complete(query: str, docs: List[SearchResult], mission):
    num = len(docs)
    if mission == 'retrieval':
        message = f"### INSTRUCTIONS ###\n\n" \
                  f"You are RankGPT, an expert ranking assistant.\n\n"\
                  f"TASK: Rank the following passages in **descending order of relevance** to the query.\n\n"\
                  f"QUERY: {query}\n"\
                  f"---PASSAGES---\n"
        ...
        message = f"### INSTRUCTIONS ###\n\n" \
                  f"You are RankGPT, an expert ranking assistant.\n\n"\
                  f"TASK: Rank the following passages in **descending order of similarity** to the reference passage.\n\n"\
                  f"REFERENCE PASSAGE: {query}\n"\
                  f"---PASSAGES---\n"
    else:
    ...
    if mission == 'retrieval':
        message += f"### RANKING FORMAT ###\n"\
                  f"- List all passage identifiers in **descending order of relevance**.\n"\
                  f"- Use the exact format: [3] > [1] > [2]\n"\
                  f"- make sure to rank all {num} passages\n"

    elif mission == 'clustering':
        message += f"### RANKING FORMAT ###\n" \
                  f"- List all passage identifiers in **descending order of similarity**.\n" \
                  f"- Use the exact format: [3] > [1] > [2]\n" \
                  f"- make sure to rank all {num} passages\n"
    return message
...
class ListwiseLlmRanker(OpenAiListwiseLlmRanker):
    ...
    def __init__(self, model_name_or_path, tokenizer_name_or_path, device, window_size, step_size,
                 scoring='generation', num_repeat=1, cache_dir=None, mission='retrieval'):
    ...
    def compare(self, query: str, docs: List):
        ...
        if self.scoring == 'generation':
            if self.config.model_type == 't5':
                input_text = create_permutation_instruction_complete(query, docs, mission=self.mission)
            ...
            if self.mission == 'retrieval':
                input_text = f'Given a query:\n\n{query}\n\nwhich of the following passages is the most relevant one to the query?\n\n' \
                             + passages + '\n\nOutput only the passage label of the most relevant passage:'
            elif self.mission == 'clustering':
                input_text = f'Given a reference passage\n\n{query}\n\nwhich of the following passages is the most similar one to the reference passage?\n\n' \
                             + passages + '\n\nOutput only the passage label of the most similar passage:'
            else:
                raise ValueError(f'no such mission: {self.mission}')
