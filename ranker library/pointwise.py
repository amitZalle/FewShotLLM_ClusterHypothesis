...
class PointwiseLlmRanker(LlmRanker):
    ...
    ### CHANGED - added mission parameter
    def __init__(self, model_name_or_path, tokenizer_name_or_path, device, method="qlm", batch_size=1, cache_dir=None, mission='retrieval'):
    ...
    ### CHANGED - added different prompt for each mission for yes_no method
    def rerank(self, query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        ...
        if self.method == "qlm":
            ...
            if self.mission == 'retrieval':
                ...
                prompt = "Passage A: {text}\nPassage B: {query}\nIs passage A similar to passage B? Answer 'Yes' or 'No'"
            else:
...
class MonoT5LlmRanker(PointwiseLlmRanker):
    ### CHANGED - added different prompt for each mission
    def rerank(self, query: str, ranking: List[SearchResult]) -> List[SearchResult]:
        self.total_compare = 0
        self.total_completion_tokens = 0
        self.total_prompt_tokens = 0
        if self.mission == 'retrieval':
            ...
            prompt = "Passage A: {query} Passage B: {document} similar:"
        else:
            raise ValueError(f"no such mission: {self.mission}")
